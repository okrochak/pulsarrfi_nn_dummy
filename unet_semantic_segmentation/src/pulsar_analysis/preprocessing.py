...
class BinarizeToMask:
    ...
    def __init__(self, binarize_func: str = "thresh"):
    ...
    def binarize_protocol(self, image: np.ndarray):
        ...
        elif self.binarizing_func == "thresh":
            #print(f'debug55: Thresh method is used')
            image_copy = BinarizeToMask.thresh_method(image=image)
            #image_copy = BinarizeToMask.thresh_ni_method(image=image)
        else:
    ...
    @staticmethod
    def thresh_method(image:np.ndarray):
        threshold = threshold_otsu(image)            
        image_copy = image            
        image_copy[image < threshold] = 0
        image_copy[image >= threshold] = 1
    ...
    @staticmethod
    def thresh_ni_method(image:np.ndarray,window_size:int=15):
        threshold = threshold_niblack(image,window_size=window_size)            
        image_copy = image            
        image_copy[image < threshold] = 0
        image_copy[image >= threshold] = 1
        return image_copy
...
class PrepareFreqTimeImage:
    ...
    def __init__(
        ...
        binarize_engine: BinarizeToMask = BinarizeToMask(binarize_func='thresh'),
    ):
